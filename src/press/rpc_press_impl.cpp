...
int PressClient::init() {
  ...
  if (_rpc_client.Init(_options->host.c_str(), _options->lb_policy.c_str(), &rpc_options) != 0) {
...
int RpcPress::init(const PressOptions *options) {
  ...
  if (!_options.output.empty()) {
    ...
    if (!butil::CreateDirectoryAndGetError(dir, &error)) {
      LOG(ERROR) << "Fail to create directory=`" << dir.value() << "', " << error;
      return -1;
    }
...
void RpcPress::handle_response(brpc::Controller *cntl, Message *request, Message *response, int64_t start_time) {
  if (!cntl->Failed()) {
    ...
    LOG(WARNING) << "error_code=" << cntl->ErrorCode() << ", " << cntl->ErrorText();
    _error_count << 1;
  }
  delete response;
  delete cntl;
...
int RpcPress::start() {
  ...
  utils::InfoThreadOptions info_thr_opt;
